...
> 这是站点偶发慢响应，同一账号重跑通常就能成功，**不是账号或密码问题**（已实测验证）。
> 默认 `login-retry: 2` 会在**全新浏览器实例**上重试，避免整批里白白少一个号。
> 每次重试间隔 `login-retry-wait` 秒，日志会打印 `登录重试 1/2（上次失败原因: ...）`。
>
> 注意：**环境类异常（浏览器起不来）不会重试** —— 那通常是机器或驱动问题，重试只会更慢，
> 由外层逐账号隔离机制处理，该账号失败但其余账号照常。

> [!IMPORTANT]
> **登录失败会明确指出卡在哪一步**，而不是笼统报「登录超时」。五种情况报错各不相同，
> 因为处理方式完全不同：
>
> | 报错 | 含义 | 怎么办 |
> | --- | --- | --- |
> | `打开首页超时（网站无响应或网络不通）` | 连不上站点 | 检查网络 / 稍后重试 |
> | `等待用户名输入框超时（页面可能未加载完或前端已改版）` | 页面没出来或改版 | 检查站点是否改版 |
> | `等待密码输入框超时（...）` | 同上，改版痕迹更明确 | 同上 |
> | `等待登录按钮超时（...）` | 同上 | 同上 |
> | `点击登录后 15s 仍未跳出登录页（账号密码错误，或服务器响应慢）` | 表单填完了但登录没成功 | **优先怀疑密码错** |
> | `页面 URL 仍停在登录页（账号密码可能错误）` | 同上 | 同上 |
>
> 这些原因会**写进失败通知**，手机上就能看到，不用翻日志。

> [!IMPORTANT]
> **两阶段抢座**：程序先逐个账号登录并拿到「轻量会话」（浏览器随之关闭），
> 之后**只发 HTTP 请求**抢座。
> 这样慢活（登录）不占用抢座窗口 —— 登录约占单账号 78% 的耗时，前置后 14 个账号的出手时间可从 **约 118 秒压缩到几秒**。

> [!IMPORTANT]
> **抢座分两轮，保证每个账号都能在开抢瞬间出手：**
>
> | 轮次 | 行为 | 目的 |
> | --- | --- | --- |
> | **第一轮** | 全部账号**各出手一次**（不重试） | 谁都不许多试，避免排在前面的账号用长时间重试把后面的账号堵在场外 |
> | **第二轮** | 只给第一轮失败的账号**补抢** | 此时才用完整的「探路 → 猛攻」节奏 |
>
> 为什么需要这样：抢座名额在开抢后几秒内就没了。如果按「一个账号从头抢到尾」的顺序执行，
> 第 1 个账号失败会占用最多 5 分钟，第 14 个账号最坏要等 **66 分钟**才出手 —— 早就结束了。
> 先全员过一轮，则所有账号都能在开抢瞬间附近摸到牌。
>
> 日志里会打印 `[第一轮]` / `[第二轮]` 便于核对。此行为由 Test 27 守住。
...
> [!TIP]
> **第 ① 步已在云端正式环境验证通过（2026-09-15）。** 用飞书表格只开启 2 个账号
> （其余 12 个设 `enabled=false`），手动触发 workflow（不勾 `dry_run`）真实抢座。
>
> | 指标 | 实测值 |
> | --- | --- |
> | 飞书读取 | 14 条记录 / 14 条密码 |
> | 登录（含冷启动） | 首号 **16.74s**（启动 7.67s + 登录 7.52s + 取UID 1.46s），次号 **10.04s**（启动 0.35s） |
> | 抢座阶段 | **6.52s**（2 账号串行，各 1 次请求即成功） |
> | 全流程总耗时 | **33.45s** |
> | 结果 | 2 成功 / 0 失败 / 12 跳过，微信通知 2 条 |
>
> **关键结论：登录完成后隔约 10 秒再发抢座请求，会话（Cookie/uid）依然有效。**
> 这是两阶段调度最大的风险点 —— 前一个账号登录完、等下一个账号登录完（约 10 秒）才开始抢，
> 凭证没有被服务器判为过期，抢座正常成功。
>
...
> 第 10 个账号 `2023014378` 报 `登录超时，仍未跳出登录页`（登录耗时 14.23s，正常约 5s）。
>
> | 观察 | 结论 |
> | --- | --- |
> | 该账号失败后，后续账号**照常处理** | 逐账号异常隔离生效，一个号挂不连累全场 |
> | 收到 `失败通知已发送` 微信推送 | 失败不会静默消失，你会被通知到 |
> | 卡住 14.23s 就放弃，未无限等待 | 超时保护生效 |
> | **同一账号单独重跑即成功** | ❗ **偶发慢响应，不是密码错或账号问题** |
>
> 正因为第 4 条，才加了 `login-retry`（默认 2 次）：这类偶发失败重试一次就能救回。
> 本次总耗时 **38.92s**，抢座阶段 5.40s，2 个目标账号均抢座成功。
...
```bash
python test_config_layering.py   # 配置分层 + 两阶段调度 + 两轮抢座 + 并发隔离 + DRY RUN + 登录重试/失败定位（27 项）
