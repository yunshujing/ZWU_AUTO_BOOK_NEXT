...
| `max-retry` | int  | 3              | 补抢轮出手次数上限（第一轮固定各 1 次，不受此值影响）       |
...
| `max-retry` | 数字     | 补抢轮出手次数上限（第一轮固定各 1 次，不受此值影响） |
...
max-retry: 3           # 补抢轮出手次数上限（第一轮固定各 1 次）
```
...
>
> 脚本**不需要**知道「几点开抢」—— 它登录完就立刻让**全部账号各出手一次**，
> 之后每 3 秒给没成功的号补一枪。
> 所以要留的是**启动 + 登录**的时间，让所有账号都能在 21:00 前后出手。
>
> 实测各环节耗时：
>
> | 环节 | 耗时 |
...
>
> ⚠️ **不建议压到 20:57 之后**：现在抢座窗口很短（单号最坏 6 秒，全部号跑完也就几十秒），
> 第一轮出手必须赶在名额被抢光之前。一旦 GitHub 排队多等一两分钟，
> 登录还没完成就已经 21:00，到时候再出手基本无望。
>
> 💡 早启动是**完全没有代价**的：第一轮每号只发 1 个请求、不重试、不等待，
> 所以在 20:55 或 21:00 出手，成本一样。
...
| `retry-probe-interval` | int | 10           | 探路间隔（秒）。**当前无轮次使用**，保留备用 |
| `retry-probe-count`  | int | 30             | 探路次数上限。**当前无轮次使用**，保留备用 |
| `retry-rush-interval` | int | 3             | 猛攻间隔（秒），补抢轮使用 |
...
| `max-retry`          | int | 3              | **补抢轮**出手次数上限（第一轮固定各 1 次，不受此值影响） |
| `notification_type`  | str | none           | 通知方式：none / wechat / email |
| `sckey`              | str | ''             | Server酱推送 Key                |

> [!TIP]
> **单账号最坏耗时** ≈ `(max-retry - 1) × retry-rush-interval`，即 **2 × 3 = 6 秒**
> （旧版固定 60 秒 × 20 次 = 20 分钟）。
>
> 为什么这么短也够用：抢座分两轮，**每个号在开抢瞬间都已经出手过一次**了
> （第一轮全员出手，不等、不重试）。补抢轮只是给没抢到的号再补几枪，
> 不需要长时间死磕 —— 名额几秒内就没了，补枪拖到几分钟后本来也没意义。
>
> ⚠️ `max-retry` 直接决定「一个号会占住后面的号多久」。串行下 14 个号全失败时：
>
> | `max-retry` | 单号耗时 | 14 号排队总时长 |
> | --- | --- | --- |
> | **3（当前）** | **6s** | **约 84 秒** |
> | 6 | 15s | 约 210 秒 |
> | 20 | 57s | 约 798 秒 |
>
> 💡 **探路参数已闲置**：原来「探路 → 猛攻」是为了应对「程序早于系统开放启动」，
> 现在这个职责由**第一轮全员出手**接管了（代价接近零），所以补抢轮直接猛攻。
> `retry-probe-*` 两个参数保留但当前无人使用。
> ⚠️ 将来若恢复探路，务必保证 `max-retry >= retry-probe-count`，
...
> | **第二轮** | 只给第一轮失败的账号**补抢**（跳过探路，直接每 3 秒一枪） | 此时才放开重试 |
>
...
> 本次因座位空闲、第 1 次尝试即成功，**重试节奏未被真正压测**，
> 如需验证重试行为，应在座位紧张时段观察。

> [!TIP]
> **抢座节奏的演进（2026-09-15 定案，三步走）：**
>
> | 阶段 | 做法 | 单号最坏耗时 |
> | --- | --- | --- |
> | 最初 | 固定 60s × 20 次 | 20 分钟 |
> | 中间 | 探路 30s × 8 + 猛攻 | 约 4.5 分钟 |
> | 中间 | 探路 10s × 30 + 猛攻 | 5 分钟 |
> | **现在** | **第一轮全员各 1 次 + 补抢轮 3 次猛攻** | **6 秒** |
>
> 关键在于「**全员先出手**」这一步：
>
> - 一开始是为了解决「14 个号逐个登录要 98 秒，排在后面的号会在两个探路点之间
>   错过 21:00 开抢瞬间」—— 于是把探路间隔从 30s 压到 10s、次数加到 30。
> - 后来发现更根本的问题是**抢座也排队**：一个号死磕完才轮到下一个，
>   最后的号最坏要等 66 分钟才出手。
> - 补上「第一轮全员各出手一次」后，**每个号在开抢瞬间都已经出过手了**，
>   探路那套「温和试探」的职责被第一轮接管 —— 于是补抢轮直接猛攻，不再等待。
>
> 结果：单号最坏耗时从 5 分钟降到 **6 秒**，同时**全员都保住了开抢瞬间的出手机会**
> （原来这两件事是矛盾的：要么全员出手但每号拖很久，要么快但后面的号排不上）。
>
> ⚠️ `retry-probe-*` 两个参数现在**无人使用**（保留备用）。将来若恢复探路，
> 务必保证 `max-retry >= retry-probe-count`，否则会被 `min()` 静默截断。
...
```bash
python test_config_layering.py   # 配置分层 + 两阶段调度 + 两轮抢座 + 并发隔离 + DRY RUN + 登录重试/失败定位（28 项）
...
├── test_config_layering.py          # 配置分层 + 两阶段调度 + 两轮抢座 + 并发隔离 + DRY RUN（28项场景）
