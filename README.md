...
>
> 密码仍然只在 GitHub Secret 里，飞书表格只存非敏感的配置项（学号、座位、时段、启用状态）。
...
多维表格右上角 **分享** 按钮旁的 `...`（或表格标题旁的 `+` 添加按钮）→ **添加文档应用** → 搜索你的应用名 → 添加并选择 **可查看** 权限
...
| Secret 名           | 值                                  | 说明                     |
| ------------------- | ----------------------------------- | ------------------------ |
| `PASSWORDS`         | `{"学号1":"密码1","学号2":"密码2"}` | 密码映射 JSON            |
...
| 场景                 | 操作                                     | 碰 Secret 吗？     |
| -------------------- | ---------------------------------------- | ------------------ |
| 加新账号             | 飞书表格加一行 + 在 `PASSWORDS` 加一条 | ✅（只加一条 key） |
| 改时段 / 座位 / 重试 | 飞书表格改对应单元格                     | ❌                 |
| 临时停用某账号       | 飞书表格取消 `enabled` 勾选             | ❌                 |
| 删账号               | 飞书表格删行（`PASSWORDS` 可留可删）   | ❌                 |
| 改密码               | 改 `PASSWORDS` Secret 对应 key         | ✅                 |

> [!NOTE]
> 飞书表格中的 `username` 必须和 `PASSWORDS` 中的 key 完全一致（含学号前导零等），否则该账号会被跳过。
